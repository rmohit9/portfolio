...
export const ExperienceView = () => {
  ...
  return (
    <div className="text-left space-y-6 animate-fadeIn md:max-w-5xl">
      ...
      <div className="relative pl-6 border-l border-[#2e303a] ml-3 space-y-8 py-2 font-mono text-sm">
        {experiences.map((job, idx) => (
          <div key={idx} className="relative">
            <div className="absolute -left-[31px] top-1.5 w-2.5 h-2.5 rounded-full bg-[#c084fc] border-4 border-[#0d1117]" />
            <div className="text-xs text-gray-500 font-bold tracking-wider">{job.period}</div>
            <div className="text-base font-bold text-[#f3f4f6] mt-1">{job.role} @ <span className="text-[#58a6ff]">{job.company}</span></div>
            <p className="text-gray-400 mt-2 leading-relaxed text-[13px]">{job.desc}</p>
          </div>
        ))}
      </div>
    </div>
