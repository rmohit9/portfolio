      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-sm">
        {contactItems.map((item, idx) => (
          <div key={idx} className="bg-[#16171d] border border-[#2e303a] p-5 rounded-xl hover:border-gray-500 transition-colors">
            <div className="font-bold uppercase tracking-wider text-xs text-gray-500" style={{ color: item.color }}>{item.medium}</div>
            <a href={item.url} target="_blank" rel="noreferrer" className="text-base text-[#f3f4f6] font-bold block mt-2 hover:underline">
              {item.path}
            </a>
          </div>
        ))}
